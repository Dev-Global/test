...
namespace KP8_VersionControl
{
    public partial class VersionControl : Form
    {
        ...
        private RecordObjectVersion QueryVersion = new RecordObjectVersion();
                        ...
                        if (answer == DialogResult.Yes)
                        {
                            ...
                            else if ((releasetype.Equals(5) || releasetype.Equals(6)) && (categorytype.Equals(1)))
                            {
                                ...
                                SaveBranch_Station infosaving = new SaveBranch_Station
                                {
                                    ...
                                    stationcode = dataStationCode[(cmbStationNo.SelectedIndex)],
                                    zonecode = Convert.ToString(ZoneCode)
                                };
                                String _serialize = new JavaScriptSerializer().Serialize(infosaving);
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            else if (cmbCategory.Text == "BY STATION")
            {
                ...
                if (cmbStationNo.Text != "")
                {
                    if (withBlock.CurrentRow == null)
                    {
                        String[] row = new[] { txtBrancCode.Text, cmbStationNo.Text.Trim(), dataStationCode[(cmbStationNo.SelectedIndex)] };
                        withBlock.Rows.Add(row);
                        return;
                    }
                    for (var i = 0; i <= withBlock.Rows.Count - 1; i++)
        ...
        private void checkText()
        {
            ...
            {
                GetDetails = new MLRestCaller((ReadIniFile.fromIniFile("GetKp8VersionControl", "GetKp8VersionControlURL")) + "getVersionForDowngrade?releasetype=" + releasetype, HttpVerb.GET);
                GetDetails.ContentType = "application/json";
                getResponse = JsonConvert.DeserializeObject<ObjectJSON>(GetDetails.MakeRequest());

                if (getResponse.respcode == "1")
                {
                    QueryVersion = JsonConvert.DeserializeObject<RecordObjectVersion>(GetDetails.MakeRequest());
                    _version = QueryVersion.recordQuery;  
                }
                else { MessageBox.Show(getResponse.message); }

                if (Convert.ToDecimal(_version) <= Convert.ToDecimal(txtVersion.Text))
                {
                    //MessageBox.Show("The current version is " + Convert.ToDecimal(_version) + Environment.NewLine + "Please input less than " + Convert.ToDecimal(_version),"Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    MessageBox.Show("The current version is " + Convert.ToDecimal(_version) +"\r\n"+ "Please input less than " + Convert.ToDecimal(_version) + " to downgrade.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    chekReqfield = false;
                    cmbAction.Focus();
                }
                else { chekReqfield = true; }
            }
            else if (cmbAction.Text.Equals("UPGRADE"))
            {
                GetDetails = new MLRestCaller((ReadIniFile.fromIniFile("GetKp8VersionControl", "GetKp8VersionControlURL")) + "getVersionForDowngrade?releasetype=" + releasetype, HttpVerb.GET);
                GetDetails.ContentType = "application/json";
                getResponse = JsonConvert.DeserializeObject<ObjectJSON>(GetDetails.MakeRequest());
                ...
                {
                    //MessageBox.Show("The current version is " + Convert.ToDecimal(_version) + Environment.NewLine + "Please input less than " + Convert.ToDecimal(_version),"Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    MessageBox.Show("The current version is " + Convert.ToDecimal(_version) + "\r\n" + "Please input greater than " + Convert.ToDecimal(_version) + " to upgrade.", "Message", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    chekReqfield = false;
                    cmbAction.Focus();
                }
                else { chekReqfield = true; }
